...
Requires Chrome on an ARCore-supported Android phone, Google Play Services for AR, and HTTPS. Choose **Play in AR** to see the full 3D pizza-box board immediately while camera tracking is available. Aim the preview, then tap the board or its raised **Place & Play** button. The thick green tracing frame indicates a detected horizontal surface; amber indicates an estimated manual position that may float above the real surface. The preview follows smoothly and placement locks its displayed transform without replacing the model. The box feet rest on detected surfaces. Detection results survive brief gaps for up to 1.5 seconds, scanning queries are limited to 20 per second, and surface queries stop after placement.

Scores float above the play area, with play/pause, restart, board sizing, repositioning, and directional arrows floating at the lower sides of the camera view. These rounded 3D panels face the viewer and remain independent of the board’s position and size. Their positions stay steady for tapping, leaving the centre clear for the game. Tap the arrows to steer in the corresponding screen direction. Pause to resize or reposition. **Screen controls** restores the on-screen joystick and controls; the exit button stays available throughout. Speed, Endless mode, player settings, and sharing remain available through the screen controls where applicable. A brief tracking interruption pauses the game but retains the last board image for up to half a second to avoid flicker. Once tracking returns, choose Resume. Longer interruptions show recovery instructions. Rendering failures return to the recoverable screen UI instead of leaving a silent blank view.
